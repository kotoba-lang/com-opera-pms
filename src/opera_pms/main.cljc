(ns opera_pms.main
  "Kotodama WASM entrypoint for the Opera_pms clean-room actor (L4) — Clojure port.
  ...
  facts (`opera_pms.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "opera_pms")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"opera_pms.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "opera_pms-compat" :tier tier :entities entities} 200])
...
(defn start! [] :opera_pms-compat/ready)
